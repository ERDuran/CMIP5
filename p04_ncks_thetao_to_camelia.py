...
for model_now in data_ls:
    ...
    output_name = model_now[:-3] + '_surf.nc'
    
    my_data_path = my_path + '/' +  output_name
    
    if output_name in my_ls:
        print(model + " is already there!")
        os.system('ncks -P -m -v lev ' + my_data_path)
        
    else:
        if model in ['HadGEM2-AO']:
            lev = 39
            print(model + " subset at lev = " + str(lev) +  "...")
            os.system('ncks -d lev,39 ' + 
