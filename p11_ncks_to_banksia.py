...
my_dir = '/g/data/e14/erd561/CMIP5/banksia'
...
for model_now in data_ls:
    ...
    output_name = model + '_' + model_now[-23:-17] + '_' + str(year_start) + '-' + str(year_finish-1) + '.nc'
    
    my_data_path = my_path + '/' +  output_name
    
    print(model + " retrieved over " + str(year_start) + '-' + str(year_finish-1) +  "...")
    if output_name in my_ls:
        print("it's already there! \n")
        
    else:
        os.system('ncks -d time,' + str(start_time) + ',' + str(finish_time) + ' ' + 
